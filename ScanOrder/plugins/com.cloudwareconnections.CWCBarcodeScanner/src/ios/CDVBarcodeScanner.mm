...
- (UIView*)buildOverlayView {
    ...
    UILabel *scannedLabel = [[UILabel alloc]initWithFrame:CGRectMake(rectArea.frame.origin.x, rectArea.frame.origin.y + rectArea.frame.size.height, rectArea.frame.size.width, 30f)];
    
    UIFont *customFont = [UIFont fontWithName:ProximaNovaSemibold size:12]; //custom font
    NSString *text = @"Scanned x";
    
    CGSize labelSize = [text sizeWithFont:customFont constrainedToSize:CGSizeMake(380, 20) lineBreakMode:NSLineBreakByTruncatingTail];

    scannedLabel.text = text;
    scannedLabel.font = customFont;
    scannedLabel.numberOfLines = 1;
    scannedLabel.baselineAdjustment = UIBaselineAdjustmentAlignBaselines; // or UIBaselineAdjustmentAlignCenters, or UIBaselineAdjustmentNone
    scannedLabel.adjustsFontSizeToFitWidth = YES;
    scannedLabel.adjustsLetterSpacingToFitWidth = YES;
    scannedLabel.minimumScaleFactor = 10.0f/12.0f;
    scannedLabel.clipsToBounds = YES;
    scannedLabel.backgroundColor = [UIColor blackColor];
    scannedLabel.textColor = [UIColor whiteColor];
    //scannedLabel.textAlignment = NSTextAlignmentLeft;
    [overlayView addSubview:scannedLabel];
